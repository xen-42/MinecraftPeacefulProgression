...
public class PeacefulProgressionEMIPlugin implements EmiPlugin {
	...
	private static void addInformationRecipes(EmiRegistry registry) {
		...
		addInfo(registry, Items.OMINOUS_BOTTLE, raidEffigy);
		addInfo(registry, Raid.getOminousBanner(registryManager.getWrapperOrThrow(RegistryKeys.BANNER_PATTERN)).getItem(), raidEffigy);
		...
		addInfo(registry, PeacefulModBlocks.BREEZE_CORAL.asItem(), gravel);
		...
		addInfo(registry, Items.TRIAL_KEY, wanderingTraderKey);
		addInfo(registry, Items.OMINOUS_TRIAL_KEY, wanderingTraderKey);
