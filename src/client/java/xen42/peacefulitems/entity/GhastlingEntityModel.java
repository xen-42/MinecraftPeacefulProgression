...
public class GhastlingEntityModel extends SinglePartEntityModel<GhastlingEntity> implements ModelWithArms {
    ...
    public void setAngles(GhastlingEntity ghastlingEntity, float limbSwingAnimationProgress, float limbSwingAmplitude, float age, float pitch, float yaw) {
        ...
        float l = MathHelper.cos(k) * (float) Math.PI * 0.15F + limbSwingAmplitude;
        ...
        this.rightWing.pitch = 0.43633232F * (1.0F - n);
        this.rightWing.yaw = (float) (-Math.PI / 4) + l;
        this.leftWing.pitch = 0.43633232F * (1.0F - n);
        this.leftWing.yaw = (float) (Math.PI / 4) - l;
        this.body.pitch = n * (float) (Math.PI / 4);

        float p = itemHoldAnimationTicks * MathHelper.lerp(n, (float) (-Math.PI / 3), -1.134464F);
        this.root.pivotY = this.root.pivotY + (float)Math.cos(m) * 0.25F * o;
        this.rightArm.pitch = p;
        this.leftArm.pitch = p;

        float q = o * (1.0F - itemHoldAnimationTicks);
        float r = 0.43633232F - MathHelper.cos(m + (float) (Math.PI * 3.0 / 2.0)) * (float) Math.PI * 0.075F * q;
        this.leftArm.roll = -r;
        this.rightArm.roll = r;
        this.rightArm.yaw = 0.27925268F * itemHoldAnimationTicks;
        this.leftArm.yaw = -0.27925268F * itemHoldAnimationTicks;
    }
